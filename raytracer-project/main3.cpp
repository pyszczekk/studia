...
int main(){
	...
	Light light(Vector(500,0,1000),1,color3[0],color3[1],color3[2]);
	ObjReader o;
	o.ReadVectors("cow.obj", 225,250,0,40);
	...
	for(uint16_t y=0; y<imgHeight3; y++){
		for(uint16_t x=0; x<imgWidth3; x++){
			...
			while(i<size){
				...
				double r,g,b;
				r=0+ light.getR()*dt*light.getIntensity();
				g=0+ light.getG()*dt*light.getIntensity();
				b=255+light.getB()*dt*light.getIntensity();
				if(r<0)r=0;
				if(g<0)g=0;
				if(b<0)b=0;
				if(r>255)r=255;
	...
	std::ofstream outfile("main3.bmp", std::ofstream::binary);
	outfile << bmp3;
